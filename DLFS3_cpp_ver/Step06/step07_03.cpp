...
class Variable{
    ...
    public:
        ...
        void Backward(Square* class_identifier){
            if(m_creator != NULL){
                Square* f = m_creator;
                // 여기서 f에 Exp가 올지 Squ가 올지 몰라서 Fun으로 둬야하는데,
                // f를 Function으로 두면 f.Get_inputted()가 아직 가상메서드 상태이기 때문에, 가상메서드 발동 오류가 뜸.
                // Square* f_square = dynamic_cast<Square*>(f);
                // if(f_square == nullptr){

                // }

                Variable& x = f.Get_inputted();
                x.Set_grad(f.Backward(m_grad));
            }
        }
};
